...
Usage examples:
  scripts/circle_area_v2.py 3
  scripts/circle_area_v2.py --radius 2.5 --precision 40
...
def compute_area(radius_str: str, precision: int = 50) -> Tuple[Decimal, int]:
    ...
    base_ctx = getcontext().copy()
    base_ctx.prec = internal_prec

    try:
        with localcontext(base_ctx):
            r = Decimal(radius_str.strip())
    ...
    # Compute area with internal precision
    with localcontext(base_ctx):
        area = (r * r) * _PI_DEC
